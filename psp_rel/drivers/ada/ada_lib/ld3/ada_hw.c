...
#if SPEAKER_GPIO_OPT
    act_writel(act_readl(GPIO_AINEN) | (1<<26), GPIO_AINEN);    /* GPIOA26 input disable; */
    act_writel(act_readl(GPIO_AOUTEN) | (1<<26), GPIO_AOUTEN);  /* GPIOA26 out enable; */
    act_writel(act_readl(GPIO_ADAT) & ~(1<<26), GPIO_ADAT);     /* GPIOA26 set low level; */
#endif
...
int dac_hw_ioctl(unsigned int cmd, unsigned int param)
{
    ...
    case DAC_HW_SPEAKER_EN:     /* enable dac speaker or not. */
        ...
        if (param)
        {
            val = act_readl(GPIO_AOUTEN); /* GPIOA26 out diable，GPIOA26 set to input, detect headphone. */
            val = val & ~(0x1 << 26);
            act_writel(val, GPIO_AOUTEN);
        }
#endif
        break;
