...
make -f makefile_fwsp_s clean

if [[ $1 = "debug" ]]
then 
read -p "sdk_api"
fi
make -f makefile_fwsp_s sdk_api

if [[ $1 = "debug" ]]
then
read -p "boot"
fi
make -f makefile_fwsp_s boot

if [[ $1 = "debug" ]]
then
read -p "os"
fi
make -f makefile_fwsp_s os

#################################################
#						 						#
#   ensure alsp make before mmm                 #
#   NOTE: sometimes alsp dir cannot be access   #
#						 						#
#################################################
...
make -f makefile_fwsp_s ko

if [[ $1 = "debug" ]]
then
read -p "pmm"
fi
make -f makefile_fwsp_s pmm

if [[ $1 = "debug" ]]
then
read -p "mmm"
fi
make -f makefile_fwsp_s mmm
...
cd $SDKROOT
#find . -name '*.so' | xargs -exec sde-strip -S -R .pdr -R .comment -R .symtab -R .strtab
find . -name '*.so' | xargs -exec sde-strip -S -R .pdr -R .comment -R .symtab -R .strtab
find . -name '*.ao' | xargs -exec sde-strip -S -R .pdr -R .comment -R .symtab -R .strtab
find . -name '*.exe' | xargs -exec sde-strip -S -R .pdr -R .comment -R .symtab -R .strtab
find . -name '*.app' | xargs -exec sde-strip -S -R .pdr -R .comment -R .symtab -R .strtab
find . -name '*.ko' | xargs -exec sde-strip -S -R .pdr -R .comment -R .symtab -R .strtab
