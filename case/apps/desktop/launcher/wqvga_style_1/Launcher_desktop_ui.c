...
//app icon style name
static unsigned int app_icon1[TYPE_COUNT] =
{
...
static unsigned int app_icon2[TYPE_COUNT] =
{
...
static unsigned int app_icon3[TYPE_COUNT] =
{
...
static unsigned int app_icon4[TYPE_COUNT] =
{
...
static void _draw_pic(int hdc, pic_data_t page_data, int x, int y)
{
    ...
    {
        sty_draw_picture_id(hdc, (int)page_data.res->pic_id, x, y);
    }
}
...
static void _draw_unsel_app_icon(appinfo_list_t *current, scene_desktop_t *data)
{
    ...
    cur_app = current;
    for (i = 0; i < (pos_offset + MAX_IN_ROW); i++) //move to 1st_app in 1st row
    {
