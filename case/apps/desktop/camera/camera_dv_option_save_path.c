 ...
 *******************************************************************************/
/*!
 * \file     camera_dv_option_save_path.c
 * \brief    概述
 * \author
...
bool camera_dv_option_save_path_enter(menuview_t *menuview)
{
    ...
    cur_menu = camera_dv_option_save_path_init();
...
menu_t* camera_dv_option_save_path_init(void)
{
    ...
    //string_resource_t *s_str_res[CAMERA_DV_OPTION_SAVE_PATH_MAX_ITEMS] = {0};
    //picture_resource_t *g_cam_dv_opt_dot;

    menuitem_t item =
    { 0 };
    //cam_dv_save_path_e num;
    int num = -1;
    s_save_opt_item_num = -1;

    //initial menu
    if (g_camera_dv_option_save_path == NULL)
    {
        //get scene
        ...
        //load resource group
        group = (resgroup_resource_t *) get_scene_child(scene, DV_STRGROUP_SAVE_PATH);
        ...
        //create menu
        g_camera_dv_option_save_path = menu_create();
        if (g_camera_dv_option_save_path == NULL)
        {
        ...
        else
        {
            ...
            g_camera_dv_option_save_path->callback = (menu_cb_t) _proc_items_do_save_path;
            ...
            if (internal_info.access == HOTPLUGIN_DEV_ACCESS_OK)
            {
                ...
                if (CAM_DV_STORAGE_INTERNAL == g_camera_dv_cfgs[CAM_DV_CFG_TYPE_SAVE_PATH].value)
                {
                    internal->sel_icon_id = (int) g_cam_dv_opt_dot->pic_id;
                    internal->unsel_icon_id = (int) g_cam_dv_opt_dot->pic_id;
                ...
                menu_append_item(g_camera_dv_option_save_path, internal);
                menuitem_delete(internal);
                /*
                另外一种做法获取高亮item
                s_save_opt_item = menu_get_cur_item(g_camera_dv_option_save_path);
            ...
            if (card_info.access == HOTPLUGIN_DEV_ACCESS_OK)
            {
                ...
                if (CAM_DV_STORAGE_CARD == g_camera_dv_cfgs[CAM_DV_CFG_TYPE_SAVE_PATH].value)
                {
                    card->sel_icon_id = (int) g_cam_dv_opt_dot->pic_id;
                    card->unsel_icon_id = (int) g_cam_dv_opt_dot->pic_id;
                    s_save_opt_item_num = num;
                }
                else
                {
                ...
                menu_append_item(g_camera_dv_option_save_path, card);
            ...
            if (external_info.access == HOTPLUGIN_DEV_ACCESS_OK)
            {
                ...
                if (CAM_DV_STORAGE_EXTERNAL == g_camera_dv_cfgs[CAM_DV_CFG_TYPE_SAVE_PATH].value)
                {
                    external->sel_icon_id = (int) g_cam_dv_opt_dot->pic_id;
                    external->unsel_icon_id = (int) g_cam_dv_opt_dot->pic_id;
                    s_save_opt_item_num = num;
                }
                else
                {
                ...
                menu_append_item(g_camera_dv_option_save_path, external);
                menuitem_delete(external);
            }

            //if (g_camera_dv_cfgs[CAM_DV_CFG_TYPE_SAVE_PATH].value == HOTPLUGIN_DEV_TYPE_NONE)
            if(num == -1)
            {
                s_save_opt_item_num = -1;
            ...
            else
            {
                ...
                tempitem = menu_get_item(g_camera_dv_option_save_path, s_save_opt_item_num);
                if (tempitem != NULL)
                {
                    tempitem->sel_icon_id = (int) g_cam_dv_opt_dot->pic_id;
                    tempitem->unsel_icon_id = (int) g_cam_dv_opt_dot->pic_id;
                }
                #endif
                if (s_save_opt_item_num == -1)
                {
                ...
                printf("choice new path = %d\n",g_camera_dv_cfgs[CAM_DV_CFG_TYPE_SAVE_PATH].value );
                //逐个动态增加最后得到高亮号
            }
            //init current index
        }
        //空条目也无所谓
        menu_set_cur_index(g_camera_dv_option_save_path, 0);

//        s_save_opt_item = menu_get_cur_item(g_camera_dv_option_save_path);

    }
    return g_camera_dv_option_save_path;

    item_err: menu_delete(g_camera_dv_option_save_path);
    return NULL;
...
int camera_dv_option_save_path_deinit(void)
{
    ...
    {
        //create menu
        menu_delete(g_camera_dv_option_save_path);
        g_camera_dv_option_save_path = NULL;
    }
...
static int _proc_items_do_save_path(menuview_t *menuview, menuview_sig_e signal)
{
    ...
    switch (signal)
    {
        case WIDGET_SIG_MENUVIEW_OK:
        {
            ...
            if(cur_item != NULL)
            {
                if (cur_item->text_id == ((int) s_str_res[CAM_DV_STORAGE_EXTERNAL-1]->str_id))
                {
                    hotplugin_dev_info_t external_info;
                    ...
                    {//不存在警告
                        ...
                        snprintf(temp_filepath, sizeof(temp_filepath), \
                            sys_get_media_dir(HOTPLUGIN_DEV_TYPE_LOCAL,DIR_VIDEO_CAMERA));
                        temp_value = CAM_DV_STORAGE_INTERNAL;
                    }
                    else
                    {
                        snprintf(temp_filepath, sizeof(temp_filepath), \
                            sys_get_media_dir(HOTPLUGIN_DEV_TYPE_UHOST,DIR_VIDEO_CAMERA));
                        temp_value = CAM_DV_STORAGE_EXTERNAL;
                    }
                }
                else if (cur_item->text_id == ((int) s_str_res[CAM_DV_STORAGE_CARD-1]->str_id))
                {
                    hotplugin_dev_info_t card_info;
                    ...
                    {
                        snprintf(temp_filepath, sizeof(temp_filepath), \
                            sys_get_media_dir(HOTPLUGIN_DEV_TYPE_CARD,DIR_VIDEO_CAMERA));
                        temp_value = CAM_DV_STORAGE_CARD;
                    }
                }
                else
                if (cur_item->text_id == ((int) s_str_res[CAM_DV_STORAGE_INTERNAL-1]->str_id))
                {
                    snprintf(temp_filepath, sizeof(temp_filepath), \
                        sys_get_media_dir(HOTPLUGIN_DEV_TYPE_LOCAL,DIR_VIDEO_CAMERA));
                    temp_value = CAM_DV_STORAGE_INTERNAL;
            ...
            print_info("dv temp path=%s", temp_filepath);


            result = camera_dv_check_path((hotplugin_dev_type_e)temp_value);
            if (result == true)
            {
                //memset((char *)dv_filepath,0,PATH_MAX);
                //memcpy(dv_filepath,temp_filepath,PATH_MAX);
                //strncpy(dv_filepath, temp_filepath, PATH_MAX);
                g_camera_dv_cfgs[CAM_DV_CFG_TYPE_SAVE_PATH].value  = temp_value;
                camera_dv_ctr_set_save_path((char *) &dv_filepath);
                camera_dv_ctr_get_disk_info(&(g_camera_scene_dv->dv_config.cfg_freeinfo));
                assert(g_camera_dv_cfgs[CAM_DV_CFG_TYPE_SAVE_PATH].value != CAM_DV_STORAGE_NONE);
                camera_dv_change_icon(CAM_ICON_STORE_TYPE, \
                (unsigned int)g_camera_dv_cfgs[CAM_DV_CFG_TYPE_SAVE_PATH].value);
                print_dbg();
            }
            else
            {
            ...
            camera_dv_widget_option_exit(menuview, camera_dv_option_save_path_enter);
#else
            menuview_set_menu(menuview, cur_menu);
            menuview_paint(menuview);
        ...
        camera_dv_option_save_path_exit(menuview);
        break;
...
{
    if ((g_camera_scene_dv->opt != NULL) && (g_camera_dv_option_save_path != NULL))
    {
        menuview_t *menuview = g_camera_scene_dv->opt;
        menu_t *cur_menu;
        camera_dv_option_save_path_exit(menuview);
        cur_menu = (menu_t *) camera_dv_option_save_path_init();
        menuview_set_menu(menuview, cur_menu);
        menuview_paint(menuview);
    }
    return true;
