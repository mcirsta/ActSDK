 ...
 *******************************************************************************/
/*!
 * \file     camera_dsc_option_special_effect.c
 * \brief    概述
 * \author
...
menu_t *g_camera_dsc_option_special_effect = NULL;

/* menu max items */
#define CAMERA_DSC_OPTION_SPECIAL_EFFECT_MAX_ITEMS   4

/* 进入主菜单 */
bool camera_dsc_option_special_effect_enter(menuview_t *menuview)
{
    menu_t *cur_menu;

    cur_menu = camera_dsc_option_special_effect_init();
...
/* 退出主菜单 */
bool camera_dsc_option_special_effect_exit(menuview_t *menuview)
{
...
menu_t* camera_dsc_option_special_effect_init(void)
{
    scene_t* scene; //!< app scene
    resgroup_resource_t *group;
    string_resource_t *str_res[CAMERA_DSC_OPTION_SPECIAL_EFFECT_MAX_ITEMS] =
    { 0 };
    ...
    //initial menu
    if (g_camera_dsc_option_special_effect == NULL)
    {
        //get scene
        ...
        //load resource group
        group = (resgroup_resource_t *) get_scene_child(scene, DSC_STRGROUP_SPECIAL_EFFECT);
        ...
        //create menu
        g_camera_dsc_option_special_effect = menu_create();
        if (g_camera_dsc_option_special_effect == NULL)
        {
        ...
        {
            // append items
            for (num = 0; num < CAMERA_DSC_OPTION_SPECIAL_EFFECT_MAX_ITEMS; num++)
            {
                if(str_res[num] != NULL)
                {                
                ...
                if (num == g_camera_dsc_cfgs[CAM_DSC_CFG_TYPE_SPECIAL_EFFECT].value)
                {
                ...
                menu_append_item(g_camera_dsc_option_special_effect, &item);
            }
        }

        //init current index
        menu_set_cur_index(g_camera_dsc_option_special_effect, 0);
    }

    return g_camera_dsc_option_special_effect;
}

int camera_dsc_option_special_effect_deinit(void)
{
    //delete menu
    if (g_camera_dsc_option_special_effect != NULL)
    {
        //create menu
        menu_delete(g_camera_dsc_option_special_effect);
        g_camera_dsc_option_special_effect = NULL;
    }
...
static int _proc_items_do_special_effect(menuview_t *menuview, menuview_sig_e signal)
{
    ...
    switch (signal)
    {
        case WIDGET_SIG_MENUVIEW_OK:
        {
            ...
            menuitem_t *save_opt_item = menu_get_item(cur_menu,
                    g_camera_dsc_cfgs[CAM_DSC_CFG_TYPE_SPECIAL_EFFECT].value);
            ...
            //g_camera_dsc_cfgs[CAM_DSC_CFG_TYPE_SPECIAL_EFFECT].value = cur_item_pos;
            //exit and save entry
            camera_dsc_ctr_set_special_effect((unsigned int) cur_item_pos);
            camera_dsc_save_parameter(CAM_DSC_PARA_SPECIAL_EFFECT, cur_item_pos);
            camera_dsc_change_icon(CAM_ICON_SPECIAL_EFFECT, (unsigned int)cur_item_pos);
#if (MENU_SELECT_OK_HIDE == 1)
            camera_dsc_widget_option_exit(menuview, camera_dsc_option_special_effect_enter);
#else
            menuview_set_menu(menuview, cur_menu);
            menuview_paint(menuview);
        ...
        camera_dsc_option_special_effect_exit(menuview);
        break;
