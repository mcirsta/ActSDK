...
bool animation_cfg_read_start_mode( void )
{
    ...
    //读取进程启动方式
    tmp = get_config( CFG_ANIMATION_POWERON_START_MODE, (char *)&start_mode, sizeof(start_mode) );
    //获取失败, 使用res方式启动
    if ( -1 == tmp )
    {
...
bool animation_cfg_read_filename( rotation_e angle, char *filename )
{
    ...
    {
        //读取当前解码文件全路径
        tmp = get_config( CFG_ANIMATION_POWERON_PIC_SIDELONG_NAME, (char *)filename, PATH_MAX );
        if ( -1 == tmp )
        {
    ...
    {
        //读取当前解码文件全路径
        tmp = get_config( CFG_ANIMATION_POWERON_PIC_UPRIGHT_NAME, (char *)filename, PATH_MAX );
        if ( -1 == tmp )
        {
            print_warning("get_config failed");
            return false;
...
bool animation_cfg_read_def_filename( rotation_e angle, char *filename )
{
    ...
    {
        //读取默认解码文件全路径
        tmp = get_config( CFG_DEF_ANIMATION_POWERON_PIC_SIDELONG_NAME, (char *)filename, PATH_MAX );
        if ( -1 == tmp )
        {
            print_warning("get_config failed");
            return false;
    ...
    {
        //读取默认解码文件全路径
        tmp = get_config( CFG_DEF_ANIMATION_POWERON_PIC_UPRIGHT_NAME, (char *)filename, PATH_MAX );
        if ( -1 == tmp )
        {
            print_warning("get_config failed");
            return false;
