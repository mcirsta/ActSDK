...
static bool _init(void)
{
    char file[PATH_MAX];
    // 获取app 的 style 资源文件名
    sprintf(file, "%s/%s.sty", sys_get_app_theme_res_dir(), "anim_on");
...
int main(int argc, const char *argv[])
{
    ...
    print_dbg("start poweron application");

    /* 应用初始化 */
    result = _lib_init();
    if(false == result)
    {
        print_err("poweron init error!\n");
        goto ANIM_POWERON_INIT_ERR;
    }
    ...
    ANIM_POWERON_APPLIB_ERR:
    _lib_deinit();

    /* 应用退出 */
    ANIM_POWERON_INIT_ERR:
    //恢复屏保, 省电
    
    print_dbg("end poweron application");
