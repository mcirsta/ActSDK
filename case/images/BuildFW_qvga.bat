...
cp -f config_qvga.txt config.txt
cp -f .\misc\lang_qvga.desktop .\misc\lang.desktop
...
if "%callfrom%"=="" (
    rem 双击运行
    ..\..\..\tools\fw_maker\FIMaker.exe fwimage_qvga.cfg
...
if "%msgfile%"=="" (
    ..\..\..\tools\fw_maker\Maker.exe -c fwimage_qvga.cfg -o %firmware%
    set retval=1
    goto make_finish
)
..\..\..\tools\fw_maker\Maker.exe -c fwimage_qvga.cfg -o %firmware% >%msgfile% 2>&1
set retval=1
:make_finish
